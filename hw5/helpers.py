...
def check_result_wrapper(actual, expected, recreate_msg=None):
    err_msg = check_result(actual, expected, recreate_msg)
...
def check_2d_iterable(actual, expected, recreate_msg):
    ...
    for outer_idx, (actual_sub, expected_sub) in enumerate(zip(actual, expected)):
        ...
        # Check that the actual value is not None
        msg = check_not_none(actual_sub, recreate_msg)
        if msg is not None:
            return f"Checking a list of lists.\nProblem at index {outer_idx} in the outer list: {msg}."

        # Check that the actual value is the correct length
        msg = check_list_length(actual_sub, expected_sub, recreate_msg)
        if msg is not None:
...
def check_2D_vals(check_val_fn, actual, expected):
    ...
    Assume actual has the right type and shape.
    """
    return None


def check_2D(check_val_fn, actual, expected):
...
def check_mask(actual, expected):
    msg = check_not_none(actual, None)
    if msg is not None:
        return msg

    type_error_msg = ("\nThe actual value does not have the correct type.\n"
                      "The expected type is List[List[bool]] \n")
    err_msg = check_2D_type(lambda val: isinstance(val, bool), actual, type_error_msg)
    if err_msg is not None:
        return err_msg

    err_msg = check_2D(lambda a, e: a == e, actual, expected)
    if err_msg is not None:
        return err_msg
...
def check_image(actual, expected):
    msg = check_not_none(actual, None)
    if msg is not None:
        return msg

    type_error_msg = ("\nThe actual value does not have the correct type.\n"
                      "The expected type is List[List[(int, int, int)]]\n")
    err_msg = check_2D_type(check_color, actual, type_error_msg)
    if err_msg is not None:
        return err_msg + "\n"
